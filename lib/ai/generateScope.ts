...
// Convert Zod schema → JSON schema for Structured Output
const scopeJsonSchema = zodToJsonSchema(ScopeDocSchema, "ScopeDoc").definitions
  ?.ScopeDoc;
...
You must generate a COMPLETE professional Scope of Work.

Rules:
- You MUST fill EVERY field in the schema.
- Arrays must not be empty.
- Be realistic, consistent, and detailed.
- Return ONLY JSON.
`;

  const user = `
Generate a scope of work from this project intake:

${JSON.stringify(intake, null, 2)}
`;

  const res = await openai.chat.completions.create({
    model: "gpt-5-mini", // mini tier GPT-5 (best cost/quality balance)
    temperature: 0.2,
    max_tokens: 3500,
    messages: [
    ...
    // 🔒 Structured Outputs: forces schema compliance
    response_format: {
      type: "json_schema",
      json_schema: {
        name: "scope_doc",
        schema: scopeJsonSchema as any,
        strict: true,
      },
    },
  });
  ...
  const validated = ScopeDocSchema.parse(parsed);
  return validated;
}
